...
from transformers import (AutoModel, AutoProcessor, Mistral3Config,
...
class Mistral3InputProcessor(InputProcessor):
    ...
    def __init__(self, model_path, model_config, tokenizer):
        ...
        hf_model_config = Mistral3Config.from_pretrained(local_model_path)
        self.dtype = hf_model_config.text_config.torch_dtype
        module_dict = nn.ModuleDict({
            "vision_tower":
            PixtralVisionModel._from_config(hf_model_config.vision_config),
            "multi_modal_projector":
            Mistral3MultiModalProjector(hf_model_config)
        })
    ...
    def _preprocess(self, images):
        result = [
            self.processor(text="dummy",
                           ...
                           return_tensors="pt",
                           device=self.device)['pixel_values'].to(
                               self.device, self.dtype) for image in images
        ]
        return result
    ...
    def _process(self, pixel_values):
        ...
        selected_image_feature = image_features[-2][:, :]
        image_features = self.mm_projector(selected_image_feature.squeeze(0),
                                           image_sizes)
...
@register_input_processor(Mistral3InputProcessor)
class Mistral3Model(PreTrainedModel):
    ...
    def __init__(self, model_config: ModelConfig[PretrainedConfig], *args,
        ...
        llm_model_config.pretrained_config = model_config.pretrained_config.get_text_config(
        )
...
AutoModel.register(Mistral3Config, Mistral3Model)
